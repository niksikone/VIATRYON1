  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-200 pb-6">
        <h1 className="text-3xl font-serif font-semibold text-[#1F2937] mb-2">Add New Product</h1>
        <p className="text-[#4B5563]">
          Upload your jewelry product and configure virtual try-on settings
        </p>
      </div>
      ...
      <form className="space-y-6" onSubmit={onSubmit}>
        ...
        <div className="bg-[#F9FAFB] rounded-lg p-6 border border-gray-200">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[#1F2937] mb-2">
                Product image URL on your website
              </label>
              <input
                className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-[#1F2937] focus:border-[#2D8C88] focus:ring-2 focus:ring-[#2D8C88]/20 outline-none transition-all"
                type="url"
                placeholder="https://yoursite.com/images/watch-1.png"
                value={websiteImageUrl}
                onChange={(event) => setWebsiteImageUrl(event.target.value)}
              />
              <p className="mt-2 text-xs text-[#4B5563]">
                Optional. Link to this product&apos;s image on your site so it&apos;s connected to your upload.
              </p>
            </div>
          </div>
        </div>
